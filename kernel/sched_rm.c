 ...
 * ====================================================================
 *
 * Filename:	$RCSfile: sched_rm.c,v $
 * Createdate:	2004/08/03 11:19:00
 * FileId:	$Id: sched_rm.c,v 1.1.1.1 2005/08/09 05:25:25 devin Exp $
...
#if CFG_KERNEL_SCHED_MAX_THREAD <=8
	return yi_g_arKeUnMapTbl[pMaskTbl->arThreadTbl1D];
#elif CFG_KERNEL_SCHED_MAX_THREAD <= 64
	n1D = yi_g_arKeUnMapTbl[pMaskTbl->arThreadTbl1D];
	return (n1D<<3) + yi_g_arKeUnMapTbl[pMaskTbl->arThreadTbl2D[n1D]];
#else
	n1D = yi_g_arKeUnMapTbl[pMaskTbl->arThreadTbl1D];
	n2D = yi_g_arKeUnMapTbl[pMaskTbl->arThreadTbl2D[n1D]];
	return (n1D<<6) + (n2D<<3) + yi_g_arkeUnMapTbl[pMaskTbl->arThreadTbl3D[(n1D<<3)+n2D]];
#endif /* CFG_KERNEL_SCHED_MAX_THREAD */
}
